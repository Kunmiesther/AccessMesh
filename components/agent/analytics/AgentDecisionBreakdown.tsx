...
export function AgentDecisionBreakdown({
  decisions,
  statuses,
}: {
  ...
  return (
    <section style={panelStyle} aria-label="Decision breakdown">
      <div style={headerStyle}>
        <div>
          <p style={eyebrowStyle}>Recommendations and outcomes</p>
          <h2 style={titleStyle}>Decision breakdown</h2>
        </div>
      </div>

      <div style={gridStyle}>
        <BreakdownCard title="Decisions" items={decisions} renderLabel={(item) => item.label} />
        <BreakdownCard title="Statuses" items={statuses} renderLabel={(item) => item.label} />
      </div>
    </section>
  );
}
...
function BreakdownCard({
  ...
  renderLabel: (item: AnalyticsBreakdownItem) => string;
}) {
  return (
          ...
          {items.map((item) => (
            <li key={item.label} style={itemStyle}>
              <div style={itemHeaderStyle}>
                <span style={itemLabelStyle}>{renderLabel(item)}</span>
